...
def assign_packages_greedy(
    ...
    """
    Pure functional greedy assignment (no OOP).
    ...
    packages = [
        {"id": 101, "weight": 6, "dest": 3},
        {"id": 102, "weight": 4, "dest": 5},
        ...
    ]

    Returns:
        assignments: {truck_id: [package_ids in assignment order]}
        unassigned: list of package_ids that could not be assigned
    """

    # ------------------------
    # 0. INIT STRUCTURES
    # ------------------------
    # Add remaining capacity + current_pos to truck dicts
    for t in trucks:
        t["remaining"] = t["capacity"]
        t["current_pos"] = depot_index

    # copy package list so we can pop from it
    remaining_packages = packages.copy()

    # map package id -> package dict for quick lookup
    package_by_id = {p["id"]: p for p in packages}

    # assignments: truck_id -> list of package ids
    assignments = {t["id"]: [] for t in trucks}

    # ------------------------
    # 1. SORT PACKAGES: heaviest first
    # ------------------------
    remaining_packages.sort(key=lambda p: (-p["weight"], p["id"]))

    def dist(u: int, v: int) -> float:
        return distance_matrix.get(u, {}).get(v, math.inf)

    # ------------------------
    # 2. GREEDY LOOP
    # ------------------------
    progress_made = True
    while remaining_packages and progress_made:
        progress_made = False

        # iterate trucks in order of max remaining capacity
        truck_order = sorted(trucks, key=lambda t: (-t["remaining"], t["id"]))

        for t in truck_order:
            best_pkg = None
            best_key = (math.inf, math.inf, "")  # (distance, -weight, id)

            for pkg in remaining_packages:
                if pkg["weight"] <= t["remaining"]:
                    d = dist(t["current_pos"], pkg["dest"])
                    key = (d, -pkg["weight"], pkg["id"])
                    if key < best_key:
                        best_key = key
                        best_pkg = pkg
    ...
    # ------------------------
    # 3. REMAINING UNASSIGNED
    # ------------------------
    unassigned = [pkg["id"] for pkg in remaining_packages]
